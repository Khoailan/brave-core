...
void KeyringService::MigrateObsoleteProfilePrefs(PrefService* profile_prefs) {
  if (profile_prefs->HasPrefPath(kBraveWalletSelectedAccountDeprecated)) {
    SetPrefForKeyring(profile_prefs, kSelectedAccountDeprecated,
                      base::Value(profile_prefs->GetString(
                          kBraveWalletSelectedAccountDeprecated)),
                      mojom::kDefaultKeyringId);
    profile_prefs->ClearPref(kBraveWalletSelectedAccountDeprecated);
  }
...
KeyringService::GetBitcoinAddresses(mojom::KeyringId keyring_id,
